...
# Default (predefined) performance metrics

The function `addDefaultMetrics` inputs a __SummarizedBenchmark__ 
object and, depending on the names of the assays, it will add
predefined performance metrics. A list of predefined performance
metrics is available upon calling the function `availableMetrics`,

```{r}
availableMetrics()
```

For example, if an assay of the
__SummarizedBenchmark__ object is called "qvalue", it will add
the metric _rejections_ by default. Check the help page for 
the list of predefined performance metrics for each assay (`?addDefaultMetrics`).

```{r}
sb <- addDefaultMetrics( sb )
names( sb@performanceMetrics[["qvalue"]] )
```
...
several ways to calculate these for each method. 

One option is to call the function _estimatePerformanceMetrics_ 
to calculate these for each performance metrics defined 
__SummarizedBenchmark__ object for each method. The parameters
for the performance functions can be passed here. For example, 
in the case below, we are specifying the performance functions 
to be estimated for several _alpha_ values.
...
By default, the function above returns a _DataFrame_, where the
...
```{r}
head( estimatePerformanceMetrics( sb, tidy=TRUE ) )
```
