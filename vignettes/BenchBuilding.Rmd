...
## Manually specifying metadata information
...
## Manually specifying package metadata
...
b <- BenchDesign(tdat) %>%
    addBMethod(blabel = "bonf",
               bfunc = p.adjust,
               bmeta = list("pkg_func" = rlang::quo(FDRreg::FDRreg),
                            "random" = 100),
               p = pval, method = "bonferroni") %>%
    addBMethod(blabel = "BH",
               bfunc = p.adjust,
...
## Multiple Metrics/Assays using BenchDesign

An example of using the multi-assay support through `bpost`.
...
We can also specify truth columns for each assay.

```{r}
sb <- buildBench(b, truthCol = c("regular" = "H", "negative" = "H"))
```
