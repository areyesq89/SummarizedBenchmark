...
Additionally, the *BenchDesign* object is also stored as part of the *SummarizedBenchmark* object, and can be accessed by simply passing the object to the `BenchDesign()` constructor. 

```{r}
BenchDesign(sb)
```

Notice, however, unlike the original *BenchDesign* above, the data is stored as `type: md5hash`. To prevent bloat when the data set is large, by default, only a MD5 hash computed using `digest::digest()` is stored with the benchmarking results. The complete data can be stored rather than just the hash by specifying `keepData = TRUE` when calling `buildBench()`. A simple helper function, `compareBDData()`, can be used to verify that while the data *type* may be different between the original *BenchDesign* and the *BenchDesign* stored in the results, the referenced data sets are the same (i.e. have matching MD5 hash values).

```{r}
compareBDData(b, BenchDesign(sb))
```
...
# Working with BenchDesigns

Before moving on to the second case study, we describe the anatomy of the *BenchDesign* class in a little bit more detail. As noted in the previous section, *BenchDesign* objects are composed of a data set and methods. Formally, the methods are stored in the *BenchDesign* as a *BDMethodList* and the data as a *BDData* object. As would be expected, the *BDMethodList* is a list (`List`) of *BDMethod* objects, each containing the definition of a method to be compared in the benchmark experiment. The general structure is illustrated in the figure below. 
...
In both design and structure, the *SummarizedBenchmark* framework is much closer to the `dscr` package. Similar to *SummarizedBenchmark*, the `r BiocStyle::Githubpkg("stephens999/dscr")` framework requires three components: data simulators, methods, and score functions. However, *SummarizedBenchmark* differs from `dscr` notably by extending the *SummarizedExperiment* class and linking benchmark metadata with results. 
