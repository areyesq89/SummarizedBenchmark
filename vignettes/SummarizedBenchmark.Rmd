...
Notice that the method names are taken from the `expandMethod()` call. To modify more than a single parameter in the duplicated methods, the new parameter values should be specified as a list. Below, both the `"contrast"` and `meta` parameters are modified in the expanded methods.
