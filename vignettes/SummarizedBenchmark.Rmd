...
Before moving to the second case study, we describe the anatomy of the *BenchDesign* and *SummarizedBenchmark* classes in a little bit more detail. 
...
Continuing with the *BenchDesign* constructed in the example above, we can access the list of methods and data set using the `BDMethodList()` and `BDData()` functions.
...
New *BDMethod* objects can be constructed with the `BDMethod()` constructor. The constructor takes the same input parameters as the `addMethod()` convenience function. 
...
Directly modifying the *BDMethodList* object provides an alternative and more modular approach to adding methods to a *BenchDesign* object than `addMethod()`.
...
The *BDData* object of a *BenchDesign* can also be similarly extracted and modified. As with methods, the data set may be removed by setting the value to NULL.
...
The helper function `hashBDData()` can be called on either *BDData* or *BenchDesign* objects to convert the data to a MD5 hash.
...
Often, benchmarking analyses are performed in multiple steps, iteratively, with new methods added. Additionally, package versions may be updated. We continue with the example of comparing methods for differential expression. Suppose we have ran the analysis a few weeks or months ago, and are interested in checking if any methods need to be rerun, e.g. due to an update in a package. The package currently provides the `updateBench()` function. We can check to see if any methods need to be rerun by simply calling on `updateBench()` with `dryrun = TRUE`. 

```{r}
updateBench(sb = sb, dryrun = TRUE)
```

Notice that none of the methods need to be rerun. Suppose we decide to 

```{r}
bd2 <- modifyMethod(bd, "deseq2", rlang::quos(bd.meta = list(note = "modified post hoc")))
updateBench(sb = sb, bd = bd2, dryrun = TRUE)
```
...
bd2 <- addMethod(bd2, "deseq2_ashr", 
                 deseq2_ashr,
                 post = list(lfc = deseq2_lfc),
                 params = rlang::quos(countData = cntdat,
                                      colData = coldat, 
                                      design = ~condition,
                                      contrast = c("condition", "2", "1")))

updateBench(sb, bd2, dryrun = TRUE)
```

We run the update by specifying `dryrun = FALSE`.
...
Additionally, we check the session metadata.

```{r}
lapply(metadata(sb2)$sessions, `[[`, "methods")
```

Also, notice that a session index is also stored in the column data of each method. This can be used to map methods quickly to the corresponding entry of the `sessions` list stored in the object metadata.

```{r}
colData(sb2)[, "session.idx", drop = FALSE]
```

We can also compare the new *SummarizedBenchmark* object against the previous *BenchDesign*.
...
Not uncommonly, a single or a small subset of methods may fail. Ideally, all other methods should continue working and be returned. By default, Failed methods are handled (verbosely) and the values in the corresponding column are set to NA.
...
bd <- BenchDesign(data = mydat) %>%
    addMethod(label = "deseq2",
              func = deseq2_run,
              post = list(pv = deseq2_pv,
                          lfc = function(x) { stop("This is bad!") }),
              params = rlang::quos(countData = cntdat,
                                   colData = coldat, 
                                   design = ~condition,
                                   contrast = c("condition", "2", "1"))
...
sbe
head(assay(sbe, "lfc"))
metadata(sbe)$sessions[[1]]$results
simplify2array(metadata(sbe)$sessions[[1]]$results)
```

Notice that while the method was running, the error message was also returned to the console in a user friendly format.

We also demostrate how this might be useful when running methods in parallel using `parallel = TRUE` and specifying a timeout limit in seconds to the `BPPARAM`.
...
assays(sbep)
metadata(sbep)$sessions[[1]]$results
simplify2array(metadata(sbep)$sessions[[1]]$results)
```
