...
b <- BenchDesign() %>%
    addMethod(
        label="kallisto-default",
        func=runKallisto,
        sample=sample,
        args="-t 16",
        meta=list( version=getKallistoVersion() )
    ) %>%
    addMethod(
        label="kallisto-bias",
        func=runKallisto,
    ...
    addMethod(
        label="salmon-default",
        func=runSalmon,
        sample=sample,
        args="-l IU -p 16",
        meta=list( version=getSalmonVersion() )
    ) %>%
    addMethod(
        label="salmon-gcBias",
        func=runSalmon,
    ...
    addMethod(
        label="sailfish-default",
        func=runSailfish,
        sample=sample,
        args="-l IU -p 16",
        meta=list( version=getSailfishVersion() )
    )
...
tidyData <- data.frame( 
  ...
  label=colData( allSB )$label[sampleIndexes] )
