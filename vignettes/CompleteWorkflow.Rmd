...
The returned object is a `SummarizedBenchmark`. The table of adjusted p-values is contained in a single "assay" of the object with each method added using `addBMethod` as a column with the corresponding `blabel` as the name.
...
The resulting object can be saved to disk with the method information tied directly to the resulting output. Additionally, comparison of the adjusted p-values can now be carried out using downstream methods using the `SummarizedBenchmark` framework.
...
As an example, we construct a `BenchDesign` object again using the differential expression example. The `BenchDesign` is created with two methods, `"deseq2"` and `"edgeR"`. Each method is specified with the optional `bmeta =` parameter.

```{r}
bdm <- BenchDesign(mydat) %>%
    addBMethod(blabel = "deseq2",
               bfunc = deseq2_pvals,
...
We can now verify that the manually defined metadata column (`meta_reason`) is available in the `colData` of the generated `SummarizedBenchmark`.
...
bdmn <- BenchDesign(mydat) %>%
    ...
    addBMethod(blabel = "edgeR",
               bfunc = edgeR_pvals,
               bmeta = list(pkg_name = packageName(environment(edgeR::glmLRT)),
                            pkg_vers = packageVersion(packageName(environment(edgeR::glmLRT)))), 
               countData = cntdat,
               group = coldat$condition,
               design = ~coldat$condition)
sbmn <- buildBench(bdmn)
colData(sbmn)
```

### Modifying Methods in BenchDesign

**An example using `modifyBMethod` function**

### Duplicating Methods in BenchDesign

**An example using `expandBMethod` function**
