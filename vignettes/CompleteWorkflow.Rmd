...
As an example, we construct a `BenchDesign` object again using the differential expression example. The `BenchDesign` is created with two methods, `"deseq2"` and `"edgeR"`. Each method is specified with the optional `bmeta =` parameter. We can verify that the manually defined metadata column (`meta_reason`) is available in the `colData` of the generated `SummarizedBenchmark`.
